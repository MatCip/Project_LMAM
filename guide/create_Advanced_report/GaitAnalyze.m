function res = GaitAnalyze(ss, L1, L2)
%   res = GaitAnalyze(s, h)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Cycle = CalcAll2(C, L1, L2)
global shanks;
L = (L1 + L2)/100;
Cycle = C;
...
for i=1:length(Cycle)
    ...
    %%%%%%%%%%%%%%%%%%%%%%% Spatial parameters %%%%%%%%%%%%%%%%%%%
     ...
     Cycle(i).StrideR = L*sqrt(2*(1-cos(Cycle(i).ShankR*pi/180)));
     Cycle(i).StrideL = L*sqrt(2*(1-cos(Cycle(i).ShankL*pi/180)));

    %%%%% stance method %%%%%%
%     alpha = naninteg(2, Cycle(i).HsR, Cycle(i).ToR)*pi/180;
%     Cycle(i).StrideR = 2*L*sin(abs(alpha(end)-alpha(1))/2);  
%     
%     if i < length(Cycle)
%         alpha = naninteg(2, Cycle(i).HsL, Cycle(i+1).ToL)*pi/180;
%     else
%         alpha = NaN;
%     end
%     Cycle(i).StrideL = 2*L*sin(abs(alpha(end)-alpha(1))/2);
    
    %%%% swing method %%%%%%%
%     alpha = naninteg(2, Cycle(i).ToR, Cycle(i).end)*pi/180;
%     Cycle(i).StrideR = L*sqrt(2-2*cos(abs(alpha(end)-alpha(1))));  
%     
%     alpha = naninteg(2, Cycle(i).ToL, Cycle(i).HsL)*pi/180;
%     Cycle(i).StrideL = L*sqrt(2-2*cos(abs(alpha(end)-alpha(1))));
